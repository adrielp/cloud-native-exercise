...
def main():
    ...
    parser.add_argument(
        "url_endpoint",
        help="The URL endpoint to check the response.")
    ...
    if ("message" and "timestamp") in data.keys():
        ...
        logger.error(
            "Response dictionary missing one or more of the expected keys.")
        sys.exit(1)
    if data["message"] == "Automate all the things!":
        logger.info(
            "The response message values is correctly set to: \"%s\"",
            data['message'])
    else:
    ...
    if re.fullmatch(reg, data["timestamp"]):
        logger.info(
            "The timestamp: \"%s\" returns matches the regex.",
            data['timestamp'])
    else:
